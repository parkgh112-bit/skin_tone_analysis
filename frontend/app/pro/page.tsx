...
import { Gem, Loader2, Search, ArrowLeft, Activity, Sparkles, Upload } from "lucide-react"
...
import MarkvuCSVUploader from "@/components/MarkvuCSVUploader"
...
export default function ProAnalysisPage() {
  ...
  const handleProAnalyze = async (rawCSVData?: any) => {
    if (!rawCSVData && !userName.trim()) {
      alert("이름을 입력해 주세요.")
      return
    }
    ...
    try {
      const payload = rawCSVData 
        ? { name: rawCSVData.Name, raw_data: rawCSVData }
        : { name: userName }
      ...
      if (rawCSVData) setUserName(rawCSVData.Name)
    } catch (error: any) {
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="text-center space-y-4">
            ...
            <p className="text-stone-600 max-w-2xl mx-auto">
              Mark-Vu 등 전문 피부 측정 장비 데이터를 기반으로 <br/>
              ...
              <Tabs defaultValue="search" className="w-full">
                <div className="flex justify-center mb-8">
                  <TabsList className="bg-stone-200/50 p-1">
                    <TabsTrigger value="search" className="gap-2 px-6">
                      <Search className="w-4 h-4" /> 이름으로 조회
                    </TabsTrigger>
                    <TabsTrigger value="upload" className="gap-2 px-6">
                      <Upload className="w-4 h-4" /> CSV 파일 업로드
                    </TabsTrigger>
                  </TabsList>
                </div>

                <TabsContent value="search" className="animate-in fade-in slide-in-from-top-4 duration-500">
                ...
                <TabsContent value="upload" className="animate-in fade-in slide-in-from-top-4 duration-500">
                  <MarkvuCSVUploader onAnalyze={(data) => handleProAnalyze(data)} />
                </TabsContent>
              </Tabs>
            </div>
          ) : (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <Button variant="outline" onClick={() => setProReportData(null)} className="gap-2">
                  <ArrowLeft className="w-4 h-4" /> 다시 조회하기
                </Button>
              ...
              {isProAnalyzing ? (
                <div className="flex flex-col items-center justify-center py-20 space-y-4">
                  ...
                  <p className="text-stone-600 font-medium">AI가 정밀 데이터를 분석 중입니다...</p>
                </div>
              ) : (
                <>
