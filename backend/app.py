...
def get_user_pro_data(user_name):
    csv_path = os.path.join(os.path.dirname(BASE_DIR), 'skin_type_result.csv')
...
def recommend_pro():
    ...
    if not user_name: return jsonify({'error': '이름 필요'}), 400
    ...
    # 2. 데이터 추출
    pro_data = get_user_pro_data(user_name)
    if not pro_data: return jsonify({'error': '데이터 없음'}), 404

    def f_v(pattern, default=0.0):
        for k, v in pro_data.items():
            if pattern.lower() in str(k).strip().lower():
    ...
    metrics = {
        ...
        "pore_small": f_v('FPoreCNT_Small_T'),
        "pore_medium": f_v('FPoreCNT_Medium_T'),
        "pore_large": f_v('FPoreCNT_Large_T'),
